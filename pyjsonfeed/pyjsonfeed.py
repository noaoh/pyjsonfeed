...
def custom_objects(d):
    return {k : v for k, v in d if k[0] == "_" and "." not in k}
...
class JSONFeed(object):
    def __init__(self, version, title, items, home_page_url=None,
                 ...
                 next_url=None, icon=None, favicon=None, author=None, expired=False,
                 hubs=None, **customs):
...
def _open_resource(url_file_stream_or_string, etag, modified, agent, referrer, handlers, request_headers):
    ...
    try:
        with open(url_file_stream_or_string, 'r') as f:
            data = f.read()
    ...
    if not isinstance(url_file_stream_or_string, str):
        url_file_stream_or_string.decode("utf-8")
